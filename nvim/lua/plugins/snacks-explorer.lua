...
local function add_actions()
  ...
  -- file diff ──────────────────────────────────────────────────
  a.explorer_codediff_compare = function(picker)
    if not vim.fn.exists(':CodeDiff') == 2 then
      return
    end

    local selected = picker:selected()
    if #selected ~= 2 then
      Snacks.notify.warn('Select exactly 2 files to compare')
      return
    end

    local file_a = Snacks.picker.util.path(selected[1])
    local file_b = Snacks.picker.util.path(selected[2])
    if selected[1].dir or selected[2].dir then
      Snacks.notify.warn('Only files can be compared, not directories')
      return
    end
